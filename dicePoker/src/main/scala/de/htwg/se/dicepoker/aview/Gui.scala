...
class Gui(controller: DPController) extends Frame {
  ...
  reactions += {
    ...
    case DiceWereRollen => {
      new FlowPanel(new Label(controller.getPlayerStarted.name + ": "), picSelection(0, controller.getPlayerStarted), picSelection(1, controller.getPlayerStarted), picSelection(2, controller.getPlayerStarted), new Button(Action("Continue") {}))
    ...
    case DeclareFirstBid => new FlowPanel(new Label(controller.playerName(controller.getPlayerStarted) + ", please declare the first bid (e.g. 3,2 /means your bid is a double of 3):"), bidInput, new Button(Action("Continue") { controller.setUserInteraction(bidInput.text) }))
    case LineSeparator   =>
    case Input           =>
    case AskIfMistrusts => {
